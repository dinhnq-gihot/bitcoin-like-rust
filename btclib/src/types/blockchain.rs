...
impl Blockchain {
    ...
    pub fn try_adjust_target(&mut self) {
        ...
        // 4
        let new_target = if new_target < self.target {
            self.target / 4
        ...
        // if the new target is mor than the minimum target set it to the minimum target
    ...
    pub fn add_to_mempool(&mut self, transaction: Transaction) -> Result<()> {
        ...
        let mut known_inputs = <HashSet<Hash>>::new();
        for input in transaction.inputs.iter() {
        ...
        for input in transaction.inputs.iter() {
            if let Some((true, _)) = self.utxos.get(&input.prev_tx_output_hash) {
                ...
                if let Some((idx, (_, referencing_transaction))) = referencing_transaction {
                    for input in referencing_transaction.inputs.iter() {
        ...
        // all inputs must not be lower than all outputs
    ...
    pub fn cleanup_mempool(&mut self) {
        let now = Utc::now();
        let mut utxo_hashes_to_unmark = Vec::<Hash>::new();
