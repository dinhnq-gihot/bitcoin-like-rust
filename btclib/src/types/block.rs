...
impl Block {
    ...
    pub fn verify_transactions(
        ...
        let mut inputs = <HashMap<Hash, TransactionOutput>>::new();

        // reject completely empty block
        if self.transactions.is_empty() {
            return Err(BtcError::InvalidTransaction);
        }

        //verify coinbase tranction
    ...
    pub fn calculate_miner_fees(
        ...
        let mut inputs = <HashMap<Hash, TransactionOutput>>::new();
        let mut outputs = <HashMap<Hash, TransactionOutput>>::new();

        // Check every transaction after coinbase
        for transaction in self.transactions.iter().skip(1) {
            for input in transaction.inputs.iter() {
                // input do not contain
                // the values of the outputs
                // so we need to match inputs to outputs
                let prev_output =
                    if let Some(o) = utxos.get(&input.prev_tx_output_hash).map(|(_, o)| o) {
                        o
                    } else {
                        return Err(BtcError::InvalidTransaction);
                    };
